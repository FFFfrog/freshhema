...
from common.libs.user.Helper import ops_render, iPagination, getCurrentDate, getDictFilterField, selectFilterObj
...
def info():
    ...
    pay_order_list = PayOrder.query.filter_by(member_id=id).filter(PayOrder.status.in_([-8, 1])).order_by(
        PayOrder.id.desc()).all()
