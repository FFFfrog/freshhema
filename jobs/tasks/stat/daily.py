...
from common.libs.user.Helper import getFormatDate, getCurrentDate
...
class JobTask():
    ...
    def statMember(self, params):
        ...
        for member_info in member_list:
            ...
            tmp_stat_pay = db.session.query(func.sum(PayOrder.total_price).label("total_pay_money")).filter(
                PayOrder.member_id == member_info.id, PayOrder.status == 1).filter(PayOrder.created_time >= date_from,
                                                                                   PayOrder.created_time <= date_to).first()
            tmp_stat_share_count = WxShareHistory.query.filter(PayOrder.member_id == member_info.id).filter(
                PayOrder.created_time >= date_from, PayOrder.created_time <= date_to).count()
    ...
    def statFood(self, params):
        ...
        stat_food_list = db.session.query(FoodSaleChangeLog.food_id,
                                          ...
                                          func.sum(FoodSaleChangeLog.price).label("total_pay_money")).filter(
            FoodSaleChangeLog.created_time >= date_from, FoodSaleChangeLog.created_time <= date_to).group_by(
    ...
    def statSite(self, params):
        ...
        stat_pay = db.session.query(func.sum(PayOrder.total_price).label("total_pay_money")).filter(
            PayOrder.status == 1).filter(PayOrder.created_time >= date_from, PayOrder.created_time <= date_to).first()
        ...
        stat_order_count = PayOrder.query.filter_by(status=1).filter(PayOrder.created_time >= date_from,
    ...
    def test(self):
        import datetime
        from common.libs.user.Helper import getFormatDate
